function initWelcomeAnimation() {
  const welcomeName = sessionStorage.getItem('rv_show_welcome');
  const overlay = document.getElementById('welcome-overlay');
  if (!welcomeName || !overlay) return;

  sessionStorage.removeItem('rv_show_welcome');
  ...
  overlay.querySelector('.welcome-backdrop')?.addEventListener('click', closeWelcome);

  setTimeout(closeWelcome, 4500);
}

function aplicarMarcaRegistroVerde() {
  document.querySelectorAll('.header-logo .logo-name, .footer-logo .logo-name').forEach((el) => {
    el.textContent = 'Registro Verde';
  });
  document.querySelectorAll('.header-logo .logo-sub, .footer-logo .logo-sub').forEach((el) => {
    el.textContent = 'AMAZÔNIA';
  });

  const footerCopy = document.querySelector('.footer-bottom p');
  if (footerCopy) {
    footerCopy.textContent = '© 2026 Registro Verde Amazônia – Todos os direitos reservados.';
  }
}

document.addEventListener('DOMContentLoaded', () => {
  lucide.createIcons();
  aplicarMarcaRegistroVerde();
  ...
  window.realizarLogin = async (event) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        const firstName = (data.user.nome || 'Guardião').trim().split(/\s+/)[0];
